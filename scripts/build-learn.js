...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: 'weixin-recall-pipeline', title: '个人微信召回系统 · 做到了哪一步，又卡在哪里', desc: '销售加了几百个好友，大半问两句就不说话了——这一页讲我们怎么把这件事交给机器，以及它今天真实的位置。架构是「服务端授权、两端执行」：Windows 销售机侧车完全被动，安卓执行器可以自己扫到人之后指名申领，但发不发由服务端判。先讲历史聊天记录为什么只能读屏幕（解密本地库与客户端 hook 两族方案都已不可用，官方会话存档覆盖不到个微），以及两端读法与成本的差别——安卓控件树优先、端上 OCR 兜底不花 API 费，Windows 历史翻页一屏一次视觉调用既是安全阀也是钱闸；再如实纠正一个容易产生的印象：「按这个人说过的话生成话术」代码在、设计完整，但今天线上跑的活动没走它，固定剧本命中时模型根本不会被调用。真正卡住「针对性」的不是生成能力是原料——两次不同口径的清点分别显示 49 位里 35 位库里一条客户发言都没有、54 位里 20 位确证从没开过口（两次快照口径不同，页面刻意没拼成一组），而被业务闸挡住的那些逐个查完建出来的任务数是 0。往下是配额与领单（常规 5 条/台/天，活动日用带过期时间的单机覆盖，09-15 实发 20、09-16 实发 40）、六道闸，以及一节「哪几道闸真的拦住过」——只列拿得出触发实例的三道，其余明写为设计意图。再往下是 GUI 自动化最尴尬的那个问题：按了回车怎么知道发出去没有（三重证据、逐字一致为什么从门槛降级成诊断字段、以及「armed 记录视为不确定时绝不重发」这条用召回量换来的规矩）。最后是企微与个微两条线的完整时间线，和三件还没做到的事——其中最要紧的一条是：企微侧有完整的回复率/到店/转化口径，而个微两端的任务表没有「是否已回复」这一列，所以这条线的效果今天只能翻日志、不能查询，本页因此一个个微侧的效果数字都不给（代码证据取自 origin/main，数据口径 2026-09-16，发布前过两轮盲审共修正 8 条事实错误）。' },
