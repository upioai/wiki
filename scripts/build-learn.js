...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: 'weixin-recall-pipeline', title: '个人微信召回系统 · 从历史聊天记录到每日自动召回', desc: '销售加了几百个好友，大半问两句就不说话了——这一页讲我们怎么把这件事交给机器：服务端做全部决策，Windows 销售机侧车和销售自己手机上的安卓执行器只当「手」。先讲历史聊天记录为什么只能读屏幕（解密本地库与客户端 hook 两条路已被平台判死，官方会话存档覆盖不到个微），以及两端读法与成本的差别；再如实纠正一个容易产生的印象——「按这个人说过的话生成话术」代码在、设计完整，但今天线上跑的活动没走它，固定剧本命中时模型根本不会被调用。真正卡住「针对性」的不是生成能力是原料：某台设备候选池里 71% 的客户一条发言都没有、37% 确证从没开过口、42 位的历史被识别垃圾写坏，把剩下 12 位逐个查完建出来的任务数是 0（且每一条都拦得对）。往下是配额与领单（常规 5 条/台/天，活动日用带过期时间的单机覆盖抬到 40）、六道闸（成交与无需求、客户原话退订、终态去重、认人不确定就不发、真发预演双闸、本地 STOP 文件）、以及 GUI 自动化最尴尬的那个问题——按了回车怎么知道发出去没有（三重证据、逐字一致为什么从门槛降级成诊断字段、以及「armed 记录视为不确定时绝不重发」这条用召回量换来的规矩）。最后是两条线的完整时间线与三件还没做到的事，其中最重要的一条是：近 36 小时 11 条召回 0 条客户回复，而这条链路仍未在真实回复上验过——所以回复率、转化率、到店率这三个数字全文一个都没给（代码证据取自 origin/main，数据口径 2026-09-16）。' },
