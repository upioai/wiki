...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: 'riemann-hypothesis', title: '黎曼猜想 · 素数深处的一条直线', desc: '全程看图、零公式：素数是数字世界的原子，它们何时出现毫无规律，整体却守着一条平滑曲线；黎曼把这件事变成一张地图，地图上「海拔为零」的点全部站在同一条直线上——167 年没人能证明。含十二条山脊切片的 ζ 地形图（只有临界线那条会一次次触到海平面）、零点航拍图与一个「只要有一个落在这里就全盘崩塌」的幽灵点，以及用黎曼显式公式真算出来的素数音乐（10 支 / 30 支音叉叠加逼近素数台阶）。后半讲为什么 10 万亿个验证仍不算证明、希尔伯特 23 题里为何只剩 8 号原地未动、证明它对现实与 Akke 分别意味着什么（顺带破除「会破解银行密码」的误解），最后拆 2026 年 8 月 Claude 把临界线下界从 41.6% 推到 67.2% 的那一仗——60 个子 agent、31M token、650 个阵亡想法、Lean 形式化验证与外部数论专家审读，以及「验证器在环」架构可直接抄的六个动作（深色技术长页，口径 2026-08-11）。' },
    ],
  },
];
