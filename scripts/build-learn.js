...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: 'weixin-recall-pipeline', title: '个人微信召回 · 读历史消息：方案、选型与两天实例标注', desc: '只讲召回链路里「读历史聊天消息」这一环。先讲方案——个人微信没有接口，解密本地库、hook 客户端、官方会话存档三条路都走不通，只剩读屏；再列两端现行选型：安卓截屏 + 端上 MLKit OCR（不出网、按气泡颜色与横坐标判谁说的），Windows 截屏 + qwen3-vl 视觉模型（每屏读两次、不一致才读第三次），以及采集与喂给话术的边界。重头是 2026-09-15/16 两天 103 条真实召回的逐条标注：按三种口径算有历史消息的比例（任一来源 18/103，逐条剔除广告识别噪声后有效 13/103），而发出去的 103 条全部是固定话题原文、0 条用到历史——因为新启用的标题通道为了把量发起来不读历史。有历史的 13 位逐条列出客户说过什么、我们回了什么、按什么策略回的：3 条不该发（含一位已在装修的客户）、8 条没接住、1 条碰巧对上；另列无历史那 90 条的三种情况与 4 次重复发送，以及标注中发现的问题（数据口径 2026-09-16 21:41，客户与销售机均已去敏）。' },
