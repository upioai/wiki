...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: 'video-workflow-landscape', title: 'AI 视频工作流全景 · 给共建方的对接速览', desc: '把团队全部 AI 视频工作流摊在一页上：一个解构端产出分镜 JSON 当契约，五条生成线只消费它——A 真人讲解（档 $6 / 实测 $1.78）、B 纯场景画外音（档 $15 / 实测 $9.92）、C 实景走位讲解（档 $30），外加试验中的逐镜点菜、小红书跨平台搬运线与仍是半成品的广告素材线。往下是四层主播数字资产（身份 / 表情 / 声音 / 至今真空的动作层）与一次事故级发现——78 张图库、14 条模特素材、9 条成片出镜的其实是三个人，memory 里却都叫同一个名字，于是身份键改成人脸 embedding，阈值 0.375 由 245 帧双峰谷底标定，「先平均再比对」被证明会双向漏报。再往下是烧录字幕的处置顺序（能擦就不要遮，四轮返工全花在遮上）、七道确定性验收闸门、n8n 十个 active 工作流撑起的生图底座，以及一本逐笔回执记出来的成本账：大脑费换模型后同批 token 从 $60.50 降到 $0.58，剩下的钱一半在图生视频、一半在一台利用率约五成的常开云工厂机，三项优化可把月支出从 $675 压到 $274。最后是四个不与主链路耦合、可独立交付独立验收的协作面（深色技术长页，成本口径 2026-08-10）。' },
