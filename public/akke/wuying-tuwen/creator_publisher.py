...
def click_publish(commit: bool = False) -> bool:
    ...
    pt = locate(
        '抖音发布页底部最右侧的【红色「发布」主按钮】(实心红色背景, 白字「发布」). '
        '注意区分: 不要选灰色的「存草稿」按钮.',
        region=(0.10, 0.78, 0.70, 1.00),
    )
    if pt is None:
        # fallback: 把 region 放大到屏幕下半部 (不含右上的预览图区), 别整屏 —
        # 整屏会被左侧菜单「发布作品」/页面顶部 logo 误判到 (例: (105,163))
        pt = locate(
            '抖音 creator 发布页【底部操作栏】里的【红色「发布」主按钮】(实心红色填充背景, '
            '白字写「发布」), 紧挨灰色「暂存离开」按钮. 注意: 不要选左侧菜单栏的"发布作品"链接, '
            '不要选页面顶部 logo, 不要选灰色「存草稿/暂存」按钮.',
            region=(0.05, 0.75, 0.75, 1.00),
        )
    if pt is None:
        return False
    if not commit:
